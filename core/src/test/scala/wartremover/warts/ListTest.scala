...
import org.brianmckenna.wartremover.warts.ListOps._

class ListTest extends FunSuite {
  test("can't use List#head on List") {
    val result = WartTestTraverser(Head) {
      println(List(1).head)
    }
  ...
  test("can't use List#tail on  List") {
    val result = WartTestTraverser(Tail) {
      println(List().tail)
    }
  ...
  test("can't use List#last on  List") {
    val result = WartTestTraverser(Last) {
      println(List().last)
    }
