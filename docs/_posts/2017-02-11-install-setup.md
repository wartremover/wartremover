...
Compile the command-line tool via `sbt "++ 2.12.11" core/assembly` and then use the provided `wartremover` shell script:

    $ sbt "++ 2.12.11" core/assembly
...
```scala
addCompilerPlugin("org.wartremover" %% "wartremover" % "2.4.7" cross CrossVersion.full)
