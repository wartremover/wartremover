...
Compile the command-line tool via `sbt "++ 2.12.19" core/assembly` and then use the provided `wartremover` shell script:

    $ sbt "++ 2.12.19" core/assembly
...
```scala
addCompilerPlugin("org.wartremover" %% "wartremover" % "3.2.0" cross CrossVersion.full)
