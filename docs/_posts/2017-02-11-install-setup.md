...
Compile the command-line tool via `sbt "++ 2.12.18" core/assembly` and then use the provided `wartremover` shell script:

    $ sbt "++ 2.12.18" core/assembly
...
```scala
addCompilerPlugin("org.wartremover" %% "wartremover" % "3.1.4" cross CrossVersion.full)
