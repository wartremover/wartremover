...
lazy val core = Project(
  ...
  assembly / assemblyOutputPath := file("./wartremover-assembly.jar"),
  (Compile / sourceGenerators) += Def.task { // Generate special continer wart "All"
    val baseFDir = (Compile / scalaSource).value
    val warts = IO.listFiles( baseFDir / "wartremover" / "warts")
       .map(_.base)
       .filterNot(skipWarts.contains)
    val content =
      s"""package org.wartremover.warts
         |// Autogenerated code, see build.sbt.
         |object All extends org.wartremover.Container {
         |  val items = List(${warts mkString ","})
         |}
         |""".stripMargin
    val file = (Compile / sourceManaged).value / "wartremover" / "warts" / "All.scala"
...
val skipWarts = Set("Unsafe", "ForbidInference")
